...
const LearningPathTimelineVisual = ({ userId }) => {
      ...
      <div className="flex">
        <aside className="w-1/5 bg-gray-200 p-4 h-screen">
          ...
          <ul>
            ...
            <li className="mb-2"><a href="/professional/LearningPathTimelineVisual">Learning Path</a></li>
          ...
          {filteredTimeline.map(course => (
            <div key={course.id} className="card mb-4">
              ...
              <div className="w-full bg-gray-300 h-2 rounded mt-2">
                <div className="bg-green-600 h-2 rounded" style={{width: `${course.progress || 0}%`}}></div>
              </div>
            </div>
          ))}
        </main>
...
export default LearningPathTimelineVisual;